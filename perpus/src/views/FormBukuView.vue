...
const save = async (values) => {
  ...
  try {
    const url = store_or_update === 'update' ? `http://localhost:8000/api/book/update/${theroute.params.theisbn}` : `http://localhost:8000/api/book/store`;
    ...
    if (response.data.success === true) {
      alertMessage.value = store_or_update === 'update' ? 'Data has been updated!' : 'Data has been saved!';
  ...
  <Form @submit="save" :validation-schema="formSchema" class="lg:ml-72" v-slot="{ errors }">
    <FormField name="isbn" v-model="form.values.isbn" v-slot="{ field }">
      <FormItem class="mb-8">
        ...
        <FormControl>
          <Input v-if="theroute.params.theisbn === undefined || -1" v-bind="field" v-model="form.values.isbn" />
          <Input v-else v-bind="field" v-model="form.values.isbn" :readonly="isbn_readonly" />
        </FormControl>
        <FormMessage />
      </FormItem>
    </FormField>
    ...
    <FormField name="pengarang" v-model="form.values.pengarang" v-slot="{ field }">
      <FormItem class="mb-8">
    ...
    <FormField name="tahun" v-model="form.values.tahun" v-slot="{ field }">
      <FormItem class="mb-8">
        ...
        <FormControl>
          <Input v-bind="field" type="number" v-model="form.values.tahun" />
        </FormControl>
        <FormMessage />
      </FormItem>
    </FormField>
    <div class="flex justify-between">
      ...
      <Button type="button" class="text-white bg-red-700" @click="therouter.push('/buku')">Cancel</Button>
    </div>
  </Form>
